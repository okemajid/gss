    ...
    <h2 className="text-white text-xl font-bold mb-6 text-center">
      ...
      <div className="w-full max-w-[1600px] mx-auto px-12">
        <div className="flex flex-row items-center justify-between gap-20">
              ...
              <>
                ...
                {statistik
                  ...
                  .map((item, i) => (
                    <button
                      ...
                      className={`col-span-2 rounded-2xl ${colors[i % colors.length].bg} ${colors[i % colors.length].text}
                        flex flex-col justify-center items-center py-6 px-5 shadow-xl hover:scale-105 transition-transform`}
                    >
                      <p className="text-7xl font-extrabold mb-3">{item.total}</p>
                ...
                {statistik
                  ...
                  .map((item, i) => (
                    ...
                    >
                      <p className="text-6xl font-extrabold mb-2">{item.total}</p>
          ...
          {/* Gambar kanan */}
          <div className="flex-[1] flex justify-center items-center md:max-h-[70vh]">
            <Image
              src="/images/hero4.png"
              alt="Ilustrasi Statistik"
              width={700}
              height={500}
              className="drop-shadow-2xl object-contain w-[100%]"
              priority
            />
          </div>
        </div>
          ...
          {statistik.map((item, i) => {
            ...
            return (
              <button
                ...
                onClick={() => setActivePopup(item)}
                className={`
                  rounded-2xl ${colors[i % colors.length].bg} ${colors[i % colors.length].text}
                  flex flex-col justify-center items-center 
